...
class CalendarController {
  ...
  List<EventsInDay> visibleEvents(DateTime dateTime) {
    if (_events == null) {
      return [];
    }
    ...
    for (int i = 0; i < _events.length; i++) {
      DateTime tempDate = new DateFormat("yyyy-MM-dd hh:mm:ss").parse(_events[i].start_date);
      if (_events[i].dateType != null && _events[i].dateType == 0) {
        if (dateTime.day == tempDate.day && dateTime.month == tempDate.month) {
          list.add(_events[i]);
        }
      } else if (_events[i].type_id == 14) {
        ...
        if (lunar.lunarDay == lunarTemp.lunarDay) list.add(_events[i]);
      } else {
        Solar solar = Solar(solarDay: tempDate.day, solarYear: tempDate.year, solarMonth: tempDate.month);
        Lunar lunarTemp = LunarSolarConverter.solarToLunar(solar);
  ...
  void _init({
    @required List<EventsInDay> events,
  ...
  DateTime _previousWeek(DateTime week) {
    setSelectedDay(_firstDayOfWeek(week.subtract(const Duration(days: 7))));
    return week.subtract(const Duration(days: 7));
  }

  DateTime _nextWeek(DateTime week) {
    setSelectedDay(_firstDayOfWeek(week.add(const Duration(days: 7))));
    return week.add(const Duration(days: 7));
  }

  DateTime _previousMonth(DateTime month) {
    if (month.month == 1) {
      setSelectedDay(DateTime(month.year - 1, 12, 1));

      return DateTime(month.year - 1, 12);
    } else {
      setSelectedDay(DateTime(month.year, month.month - 1, 1));
  ...
  DateTime _nextMonth(DateTime month) {
    if (month.month == 12) {
      setSelectedDay(DateTime(month.year + 1, 1, 1), runCallback: true);
