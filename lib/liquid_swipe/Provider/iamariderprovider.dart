...
import 'package:calendar/liquid_swipe/Helpers/Helpers.dart';
import 'package:calendar/liquid_swipe/Helpers/slide_update.dart';
import 'package:calendar/liquid_swipe/PageHelpers/animated_page_dragger.dart';
...
class IAmARiderProvider extends ChangeNotifier {
  ...
  animateDirectlyToPage(int page, int duration) {
    ...
    new Timer.periodic(const Duration(milliseconds: 1), (t) {
      if (t.tick < duration / 2) {
        updateSlide(SlideUpdate(SlideDirection.rightToLeft, t.tick / duration, 1, UpdateType.dragging));
      } else if (t.tick < duration) {
        updateSlide(SlideUpdate(SlideDirection.rightToLeft, t.tick / duration, 1, UpdateType.animating));
      } else {
        updateSlide(SlideUpdate(SlideDirection.rightToLeft, 1, 1, UpdateType.doneAnimating));
        t.cancel();
        isInProgress = false;
      }
    });
  ...
  animateToPage(int page, int duration) {
    ...
    if (activePageIndex < page) {
      ...
      new Timer.periodic(Duration(milliseconds: newDuration), (callback) {
        new Timer.periodic(const Duration(milliseconds: 1), (t) {
          if (t.tick < newDuration / 2) {
            updateSlide(SlideUpdate(SlideDirection.rightToLeft, t.tick / newDuration, 1, UpdateType.dragging));
          } else if (t.tick < newDuration) {
            updateSlide(SlideUpdate(SlideDirection.rightToLeft, t.tick / newDuration, 1, UpdateType.animating));
          } else {
            updateSlide(SlideUpdate(SlideDirection.rightToLeft, 1, 1, UpdateType.doneAnimating));
            t.cancel();
          }
      ...
      new Timer.periodic(Duration(milliseconds: newDuration), (callback) {
        new Timer.periodic(const Duration(milliseconds: 1), (t) {
          if (t.tick < newDuration / 2) {
            updateSlide(SlideUpdate(SlideDirection.leftToRight, t.tick / newDuration, 1, UpdateType.dragging));
          } else if (t.tick < newDuration) {
            updateSlide(SlideUpdate(SlideDirection.leftToRight, t.tick / newDuration, 1, UpdateType.animating));
          } else {
            updateSlide(SlideUpdate(SlideDirection.leftToRight, 1, 1, UpdateType.doneAnimating));
            t.cancel();
          }
        });
        if (callback.tick >= diff) {
  ...
  jumpToPage(int page) {
    ...
    updateSlide(SlideUpdate(SlideDirection.rightToLeft, 1, 0.5, UpdateType.doneAnimating));
    isInProgress = false;
  }
  ...
  updateData(SlideUpdate event) {
    ...
    if (_slidePercentCallback != null && event.updateType != UpdateType.doneAnimating) {
      String hor = (event.slidePercentHor * 100).toStringAsExponential(2);
      String ver = (event.slidePercentVer * 100).toStringAsExponential(2);
      _slidePercentCallback(double.parse(hor), (((double.parse(ver)) * 100) / 125));
    }
    ...
    if (event.updateType == UpdateType.dragging) {
      ...
      if (slideDirection == SlideDirection.leftToRight && activePageIndex != 0) {
        nextPageIndex = activePageIndex - 1;
      } else if (slideDirection == SlideDirection.rightToLeft && activePageIndex != pagesLength - 1) {
        nextPageIndex = activePageIndex + 1;
      }
      return;
    }
